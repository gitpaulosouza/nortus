...
class _NewsSearchBarState extends State<NewsSearchBar> {
  ...
  void _performSearch() {
    ...
    if (query.isNotEmpty) {
      context.read<NewsBloc>().add(NewsSearchQueryChanged(query));
    }
  }

  void _clearSearch() {
    _searchController.clear();
    context.read<NewsBloc>().add(const NewsSearchQueryChanged(''));
    setState(() {
      _isEditing = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<NewsBloc, NewsState>(
      builder: (context, state) {
        if (state.searchQuery.isNotEmpty && !_isEditing) {
